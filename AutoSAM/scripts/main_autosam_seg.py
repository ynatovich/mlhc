...
import sys
sys.path.append('./../../')
from AutoSAM.loss_functions.dice_loss import SoftDiceLoss
from AutoSAM.loss_functions.metrics import dice_pytorch, SegmentationMetric

from AutoSAM.models import sam_seg_model_registry
from AutoSAM.dataset import generate_dataset, generate_test_loader, generate_dataset_miccai
from AutoSAM.evaluate import test_synapse, test_acdc, test_brats
...
parser.add_argument('--num_samples_train', default=None, type=int,
                    help='number of train samples for miccai.')
parser.add_argument('--num_samples_val', default=None, type=int,
                    help='number of val samples for miccai.')
parser.add_argument('--num_samples_test', default=None, type=int,
                    help='number of test samples for miccai.')
...
parser.add_argument('--validate_flag', type=bool, default=False)
parser.add_argument('--aug_type', type=str, default='difficult', help='difficult or easy')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.model_type == 'vit_h':
        model_checkpoint = './../../sam/sam_vit_h_4b8939.pth'
    ...
    if args.dataset == 'MICCAI':
        train_loader, train_sampler, val_loader, val_sampler, test_loader, test_sampler = generate_dataset_miccai(args)
    else:
    ...
    filename = os.path.join(args.save_dir, f'checkpoint_num_samples_{args.num_samples_train}.pth.tar')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        if args.dataset == 'MICCAI':
            train_miccai(train_loader, model, optimizer, scheduler, epoch, args, writer)
            loss = validate_miccai(val_loader, model, epoch, args, writer)
        else:
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best=is_best,args=args, filename=filename)
    if args.dataset == 'MICCAI':
        # run_test_miccai(model,test_loader, args)
        test_loss = run_test_miccai(test_loader, model, args, writer)
    else:
        run_test(model, args)
...
def train_miccai(train_loader, model, optimizer, scheduler, epoch, args, writer):
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    dice_loss = SoftDiceLoss(batch_dice=True, do_bg=False)
    ...
    for i, sample in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        tup = sample[-2].repeat(1, 3, 1, 1), sample[-1]
        if args.gpu is not None:
            img = tup[0].float().cuda(args.gpu, non_blocking=True)
            label = tup[1].long().cuda(args.gpu, non_blocking=True)
        else:
        ...
        pred_softmax = F.softmax(mask, dim=1)
        loss = ce_loss(mask, label.squeeze(1)) + dice_loss(pred_softmax, label.squeeze(1))
        # loss = ce_loss(mask, label.squeeze(1)) + dice_loss(pred_softmax, label)
        # + dice_loss(pred_softmax, label.squeeze(1))
...
def validate_miccai(val_loader, model, epoch, args, writer):
    dice_loss_list = []
    dice_list = []
    dice_loss = SoftDiceLoss(batch_dice=True, do_bg=False)
    model.eval()
    ...
    with torch.no_grad():
        for i, sample in enumerate(val_loader):
            ...
            pred_softmax = F.softmax(mask, dim=1)
            loss = dice_loss(pred_softmax, label.squeeze(1))  # self.ce_loss(pred, target.squeeze())
            dice_loss_list.append(loss.item())

    print('Validating: Epoch: %2d Loss(DiceLoss): %.4f IoU_pred: %.4f' % (epoch, np.mean(dice_loss_list), iou_pred.item()))
    writer.add_scalar("val_loss", np.mean(dice_loss_list), epoch)
...
def run_test_miccai(test_loader, model, args, writer):
    dice_loss_list = []
    dice_list = []
    dice_loss = SoftDiceLoss(batch_dice=True, do_bg=False)
    model.eval()
    ...
    print('Testing: Loss(DiceLoss): %.4f IoU_pred: %.4f' % (np.mean(dice_loss_list), iou_pred.item()))
    writer.add_scalar("test_loss(dice_loss)", np.mean(dice_loss_list))
    return np.mean(dice_loss_list)


def run_test_2(data_loader, model, args):
    print('Test')
...
def save_checkpoint(state, is_best, args, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        torch.save(state, filename)
        shutil.copyfile(filename, f'model_best_sample_num_{args.num_samples_train}.pth.tar')
