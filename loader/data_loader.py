...
class ImageFolder_new(data.Dataset):
	def __init__(self, seg_list, GT_list,image_list,image_size=512,mode='train',augmentation_prob=0.4,load_preseg = False):
		...
		self.CropRange = [400,519]  # Note that the upper bound does not seem to be greater than the lower bound of resize, to be verified
	...
	def __getitem__(self, index):
		...
		Transform = []
		Transform_GT = []  # Note that the interpolation of GT requires the nearest neighbor, but non-linear interpolation may have a miraculous effect
		...
		if (self.mode == 'train') and p_transform <= self.augmentation_prob:
			Transform.append(
				T.Resize((int(ResizeRange * aspect_ratio), ResizeRange), interpolation=Image.BICUBIC))  # bicubic
			Transform_GT.append(
				T.Resize((int(ResizeRange * aspect_ratio), ResizeRange), interpolation=Image.NEAREST))  # nearest neighbor
			...
			# On the basis of the large rotation interval, adjust the rotation angle slightly
...
class ImageFolder_new_difficult(data.Dataset):
	...
	def __getitem__(self, index):
		...
		if (self.mode == 'train') and p_transform <= self.augmentation_prob:
			...
			# Transform_GT =[]
			"""
			# ----------------------------------- Shaonan's amplification operation ------------------------------------
...
def get_loader(seg_list,GT_list,image_list,
			   ...
			   load_preseg = False, num_workers=1, mode='train',augmentation_prob=0.4):
