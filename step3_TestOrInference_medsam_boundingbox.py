...
from MedSAM.segment_anything import SamPredictor, sam_model_registry, SamAutomaticMaskGenerator
        ...
        for index, img_file in enumerate(test_img_list):
            ...
            with torch.no_grad():
                ...
                with torch.no_grad():
                    ...
                    # mask_c1_array_biggest = largestConnectComponent(sam_masks.astype(np.int))
                ...
                with torch.no_grad():
                    ...
                    sam_masks, quality_prediction,_ = sam_predictor.predict(box=np.array([dim1_cut_min,dim2_cut_min,dim1_cut_max,dim2_cut_max]))
