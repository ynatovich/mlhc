...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for index, img_file in enumerate(test_img_list):
            print(task_name, '\n', img_file, index + 1, '/', len(test_img_list), ' c1:', c1_size, ' c2: ')
            with torch.no_grad():
                # Read GT data
                if mask_path is not None:
                ...
                with torch.no_grad():
                    sam_masks = sam_mask_generator.generate(img_uint8)
                    top_iou = torch.argmax(torch.tensor([x["predicted_iou"] for x in sam_masks]))
                    sam_mask = sam_masks[top_iou]["segmentation"]
                sam_mask = sam_mask.astype(np.float32)
                final_mask = np.zeros(shape=or_shape, dtype=sam_mask.dtype)
                sam_mask = resize(sam_mask, cut_image_orshape, order=1)
                final_mask[location[0]:location[1], location[2]:location[3]] = sam_mask
