...
class Solver(object):
	def __init__(self, config, train_loader, valid_loader, test_loader, encoder_name = "efficientnet-b7", encoder_weights = "imagenet"):
		# Make record file
		...
		# Set learning rate strategy related parameters
		self.decay_ratio = config.decay_ratio
		self.lr_cos_epoch = config.lr_cos_epoch
		self.lr_warm_epoch = config.lr_warm_epoch
		self.lr_sch = None # Initialization is first set to None
		self.lr_list = [] # temporary record lr


		#
		self.tta_mode = config.tta_mode
		# execute an initialization function
		self.my_init()
	...
	def build_model(self):
		...
		# Build your own network (network structure) here
		import segmentation_models_pytorch_4TorchLessThan120 as smp
		self.unet = smp.DeepLabV3Plus(encoder_name = encoder_name,
		...
		# lr schachle strategy (to be passed into the optimizer)
		# There are three situations temporarily, (1) only use cosine decay, (2) only use warmup, (3) use both
		if self.lr_warm_epoch != 0 and self.lr_cos_epoch == 0: # zhishiyong
			self.update_lr(self.lr_low) # To use warmup, you need to initialize lr to the minimum lr
	...
	def train(self):
		...
		# Continue training to see if there is an optimal model saved during the previous training
		...
		for epoch in range(self.num_epochs):
			...
			# Record lr to the log (and record it to the picture)
			current_lr = self.optimizer.param_groups[0]['lr']
			print(current_lr)
			...
			# Learning Rate Policy Section =========================
			# lr scha way 1:
			if self.lr_sch is not None:
				if (epoch+1) <= (self.lr_cos_epoch+self.lr_warm_epoch):
					self.lr_sch.step()

			# lr scha way 2: Decay learning rate(If you use method 1, do not use this method)
			if self.lr_sch is None:
		...
		with torch.no_grad():
			for i, sample in enumerate(data_lodear):
				...
				for ii in range(SR.shape[0]):
					...
					GT_tmp = GT[ii, :].reshape(-1)
					tmp_index = images_path[ii].split('/')[-1]
					tmp_index = tmp_index.replace("fft", "3000")
					tmp_index = int(tmp_index.split('.')[0][:])
